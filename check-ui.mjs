...
await page.click('#tab-workflow');await page.click('[data-work-scenario="2"]');await page.fill('#editor-text','<h1>My first build</h1>');await page.click('#editor-save');await page.fill('#work-command', 'make build');await page.locator('#work-command').press('Enter');await page.fill('#work-command', 'make build');await page.locator('#work-command').press('Enter');assert.match(await page.locator('#work-output').innerText(),/Nothing to be done/);await page.fill('#work-command', 'make serve');await page.locator('#work-command').press('Enter');assert.equal(await page.frameLocator('#work-preview').locator('h1').innerText(),'My first build');assert.match(await page.locator('#work-feedback').innerText(),/Complete/);
await page.click('[data-work-scenario="3"]');await page.fill('#editor-text','<p>broken</p>');await page.click('#editor-save');await page.fill('#work-command','bun run test');await page.locator('#work-command').press('Enter');assert.match(await page.locator('#work-output').innerText(),/FAIL/);assert.equal(await page.locator('#work-preview').isHidden(),true);
await page.click('#work-reset');await page.click('[data-work-scenario="2"]');assert.equal(await page.locator('#work-exit').innerText(),'exit 0');await page.fill('#editor-text','<h1>Unsaved draft</h1>');await page.selectOption('#editor-file','Makefile');await page.selectOption('#editor-file','src/index.html');assert.equal(await page.locator('#editor-text').inputValue(),'<h1>Unsaved draft</h1>');await page.click('#work-reset');await page.click('[data-work-scenario="2"]');await page.screenshot({path:'/private/tmp/arise-workflow.png',fullPage:true});
