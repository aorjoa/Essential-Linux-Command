...
import {scenarioFiles} from './dist/workflow-scenarios.mjs';
...
try{
 ...
 async function download(folder){
  ...
  assert.equal(result.suggestedFilename(),'try-makefile.zip');const zip=join(temp,folder+'.zip');await result.saveAs(zip);
  execFileSync('unzip',['-t',zip]);execFileSync('unzip',['-q',zip,'-d',join(temp,folder)]);return join(temp,folder,'try-makefile');
 }
 const roots=[];
 for(let i=0;i<5;i++){
  await page.click(`[data-work-scenario="${i}"]`);const expected=scenarioFiles(base,i),paths=Object.keys(expected);
 ...
 const chainRoot=roots[4];await writeFile(join(chainRoot,'scripts/dev.sh'),'#!/bin/sh\nbun run format && bun run test && bun run start\n');
 async function serve(root,command,args){
  ...
  assert.match(await (await fetch(url)).text(),/Hello, try-makefile!/);assert.equal((await fetch(url+'missing')).status,404);
  process.kill(-server.pid,'SIGTERM');await new Promise(resolve=>server.once('exit',resolve));server=null;
 }
 await serve(webRoot,bun,['run','start']);await serve(chainRoot,'sh',['scripts/dev.sh']);
 const built=await readFile(join(chainRoot,'dist/index.html'),'utf8');await writeFile(join(chainRoot,'src/index.html'),'<p>Broken</p>   ');
 assert.throws(()=>execFileSync('sh',['scripts/dev.sh'],{cwd:chainRoot,encoding:'utf8',stdio:'pipe',timeout:5000}),error=>error.status===1&&/Formatted[\s\S]*FAIL/.test(error.stdout)&&!/Built|running at/.test(error.stdout));assert.equal(await readFile(join(chainRoot,'dist/index.html'),'utf8'),built);
